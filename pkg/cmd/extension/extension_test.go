...
func writeGitHelperScript(t *testing.T, target string) {
	...
	if runtime.GOOS == "windows" {
		...
		script := "@echo off\r\n" +
			"set GO_WANT_EXTENSION_GIT_HELPER_PROCESS=1\r\n" +
			"\"" + exe + "\" -test.run=TestExtensionGitHelperProcess -- %*\r\n" +
			"exit /b %ERRORLEVEL%\r\n"
...
func TestExtensionGitHelperProcess(t *testing.T) {
	if os.Getenv("GO_WANT_EXTENSION_GIT_HELPER_PROCESS") != "1" {
		return
	}

	args := extensionTestArgsAfterDoubleDash(os.Args)
	if err := os.WriteFile(os.Getenv("EXTENSION_GIT_ARGS_FILE"), []byte(strings.Join(args, "\n")), 0o644); err != nil {
		panic(err)
	}
	os.Exit(0)
}

func extensionTestArgsAfterDoubleDash(args []string) []string {
