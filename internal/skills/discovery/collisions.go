...
// would be installed to the same directory) and returns them sorted by name.
// Skills are installed flat by Name, so two skills with the same Name but
// different Namespace values still conflict.
...
// FormatCollisions renders collisions as "name: display1, display2" lines
// separated by newlines with leading indentation, for error messages.
